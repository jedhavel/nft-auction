...
type TransferNft struct {
	...
	SenderID         string `json:"ownerID,required"`
	SenderAESKey     string `json:"ownerAESKey,required"`      // This is generated by the AES Algorithms
...
func transferNft(stub shim.ChaincodeStubInterface, args []string) cycoreutils.Response {
	...
	isNftOnAuction, err := verifyIfNFTIsOnAuctionByUser(stub, transNft.NftId, transNft.SenderID)
	if err != nil {
	...
	userBytes, err := cycoreutils.QueryObject(stub, USER, []string{transNft.SenderID}, collectionName)
	if err != nil {
	...
	userBytes, err = cycoreutils.QueryObject(stub, USER, []string{transNft.Transferee}, collectionName)
	if err != nil {
	...
	recieverIndex, err = validateNftOwnership(stub, transNft.NftId, transNft.receiver, transNft.receiverAesKey)
	if err != nil {
		return cycoreutils.ConstructResponse("SASTQRY008E", err.Error(), nil)
	}

	if ownerIndex >= 0 { // then the receiver already has some of them
		nftObject.NftStatus = INITIAL
		nftObject.owners[recieverIndex].aesKey = transNft.TransfereeAESKey
		nftObject.ItemImage = transNft.ItemImage //TODO Why?
		nftObject.owners[recieverIndex].shareHolder = transNft.receiver
		nftObject.owners[recieverIndex].quantity += transNft.quantity
	} else { // new nftShare record
		nftObject.owners = append(nftObject.owners, NftShare{transNft.receiver, transNft.quantity, transNft.TransfereeAESKey})
	}
	nftObject.owners[senderIndex].quantity -= transNft.quantity // Dont forget to decrement the senders quantity
...
func validateNftOwnership(stub shim.ChaincodeStubInterface, nftId string, owner string, AESKey string) (int, error) {
	...
	keyValidated := false
	for o := range nftObject.owners {
		if o.holderAesKey == AESkey {
			keyValidated = true
		}
	}
	if !keyValidated {
